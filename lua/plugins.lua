...
return packer.startup(function()
	...
	use({ "zhimsel/vim-stay", event = "BufRead" }) -- Automatic fold persistance
	use({
	...
	use({ "lewis6991/impatient.nvim" }) -- Improve startup time (TODO: check for merged PR)
	use({
	...
	use({ "machakann/vim-sandwich", keys = { "s" } }) -- A more powerful and extensible surround,
	use({ "tommcdo/vim-exchange", keys = "cx" }) -- Text exchange operator (cx)
	use({
		"mizlan/iswap.nvim",
		module = "iswap",
		config = function()
